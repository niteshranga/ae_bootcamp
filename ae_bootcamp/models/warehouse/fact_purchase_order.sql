...
from {{ref('stg_purchase_order')}} po 
left join {{ref('stg_purchase_order_details')}} pod on po.id = pod.purchase_order_id 
left join {{ref('stg_products')}} pd on pod.product_id = pd.id
left join {{ref('stg_order_details')}} od on od.product_id = pd.id
